...
def invalidate(conn, project_id):
    conn.execute(
        "UPDATE projects SET revision=revision+1, status='draft', result=NULL, error=NULL, progress=0 WHERE id=?",
        (project_id,),
    )
...
def run_analysis(project_id):
    ...
    try:
        ...
        message = (
            ...
            else "Не удалось завершить анализ. Проверьте документы и повторите запуск."
        )
    ...
    with db() as conn:
        ...
        for row in conn.execute("SELECT * FROM projects ORDER BY created_at DESC"):
            ...
            item["document_count"] = conn.execute(
                "SELECT COUNT(*) FROM documents WHERE project_id=?", (item["id"],)
            ).fetchone()[0]
...
def detail(project_id: str):
    with db() as conn:
        ...
        project["documents"] = [
            document_dict(row)
            for row in conn.execute(
                "SELECT * FROM documents WHERE project_id=? ORDER BY phase,created_at",
                (project_id,),
            )
        ]
        project["runs"] = [
            dict(row)
            for row in conn.execute(
                "SELECT id,created_at,revision FROM runs WHERE project_id=? ORDER BY created_at DESC",
                (project_id,),
            )
        ]
        return project
...
def history(project_id: str, run_id: str):
    with db() as conn:
        row = conn.execute(
            "SELECT result FROM runs WHERE id=? AND project_id=?", (run_id, project_id)
        ).fetchone()
        if not row:
